...
void MuJoCoSimulator::setActuatorCommands(const swerve_chassis::WheelCommand wheels[3]) {
    ...
    // 临时注释掉舵轮协调，直接使用原始轮速
    /*
    const double STEER_ERROR_THRESHOLD = 0.52;  // 30度，舵角误差阈值（更宽容）
    for (int i = 0; i < 3; i++) {
        // 计算舵角误差（当前舵角 vs 目标舵角）
        ...
        if (abs_error > STEER_ERROR_THRESHOLD) {
            // 误差 > 30° 时，轮速为 50%（提高最低速度）
            wheel_speed_scale = 0.5;
        } else {
            // 误差 0-30° 之间，从 50% 线性增长到 100%
            wheel_speed_scale = 0.5 + 0.5 * (1.0 - abs_error / STEER_ERROR_THRESHOLD);
        }
    ...
    */
    ...
    if (++counter % 100 == 0) {
        printf("=== 状态 [%.1fs] ===\n", counter * 0.002);
        printf("舵角目标: %.1f° %.1f° %.1f°\n",
               data_->ctrl[0] * 180.0 / M_PI,
               data_->ctrl[1] * 180.0 / M_PI,
               data_->ctrl[2] * 180.0 / M_PI);
        printf("舵角实际: %.1f° %.1f° %.1f°\n",
               data_->qpos[3] * 180.0 / M_PI,
               data_->qpos[5] * 180.0 / M_PI,
               data_->qpos[7] * 180.0 / M_PI);

        // 显示轮速信息（舵轮协调已禁用）
        for (int i = 0; i < 3; i++) {
            double current_steer = data_->qpos[3 + i*2];
            double target_steer = data_->ctrl[i];
            double steer_error = target_steer - current_steer;
            while (steer_error > M_PI) steer_error -= 2 * M_PI;
            while (steer_error < -M_PI) steer_error += 2 * M_PI;
            double abs_error = std::abs(steer_error);

            printf("  轮%d: 误差%.1f° 原始轮速%.1f 最终%.1f (协调已禁用)\n",
                   i+1, abs_error * 180.0 / M_PI,
                   wheels[i].wheel_speed,
                   adjusted_wheels[i].wheel_speed);
        }
